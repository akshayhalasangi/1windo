...
class User_autologin extends W_Model
{
    ...
    public function get($user_id, $key)
    {
        ...
        $user = $this->db->get('autologin')->row();
        ...
        $this->db->from($table);
        $this->db->join('autologin', 'autologin.user_id = ' . $table . '.' . $_id);
        $this->db->where('autologin.user_id', $user_id);
        $this->db->where('autologin.key_id', $key);
        $query = $this->db->get();
    ...
    {
        return $this->db->insert('autologin', array(
            'user_id' => $user_id,
            'key_id' => $key,
