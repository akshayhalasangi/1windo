...
class Perfex_Base
{
    ...
    // Currently reminders 
    private $available_reminders = array('client', 'lead', 'estimate', 'invoice', 'proposal', 'expense');
    ...
    function __construct()
    {
        ...
        $options = $this->_instance->db->get('options')->result_array();
    ...
    public function get_option($name)
    {
        ...
        if (isset($this->options[$name])) {
            if (in_array($name, $this->dynamic_options)) {
                ...
                return $this->_instance->db->get('options')->row()->value;
            } else {
    ...
    public function get_contact_permissions()
    {
        $permissions = array(
            array(
                'id' => 1,
                'name' => _l('client_permission_invoice'),
                'short_name' => 'invoices'
            ),
            array(
                'id' => 2,
                'name' => _l('client_permission_estimate'),
                'short_name' => 'estimates'
            ),
            array(
                'id' => 3,
                'name' => _l('client_permission_contract'),
                'short_name' => 'contracts'
            ),
            array(
                'id' => 4,
                'name' => _l('client_permission_proposal'),
