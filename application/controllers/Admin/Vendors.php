...
class Vendors extends Admin_Controller
{
    ...
    public function Vendor_edit_bckup($id = '')
    { //Vendor Edit Functionality which will not be used for now. Its for CRUD Operation of vendor
    ...
    public function DeleteVendor()
    {
        ...
        $AuthenticationArray = $this->Authentication_model->AppGet(null,
            array('M_Type' => '2', 'RelationID' => $VendorId));
...
// exit;
    ...
    public function VendorProducts($id = '')
    {
        ...
        if ($id != '') {
            ...
            $result = $this->db->get('1w_tbl_vendors')->row();
            $this->db->where('P_CategoryID', $result->V_CategoryID);
            ...
            $remainingProducts = array_filter($allproducts, function ($product) use ($vendorProductsIds) {

                return !in_array($product['ProductID'], $vendorProductsIds);
            });
            $data['productsList'] = $remainingProducts;
            $data['productID'] = $id;
            $data['listAssets'] = 'true';

            $this->load->view('admin/vendors/vendorproduct', $data);
        }
    }

    public function AddProductForVendor()
    {
    ...
    public function ListVendorProducts($id = '')
    {
        if ($id != '') {
            $this->db->where('vendor_id', $id);
            $vendorProducts = $this->db->get('1w_tbl_product_vendor')->result_array();
